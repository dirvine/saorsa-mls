...
fn test_group_config_has_rekey_policy() {
    ...
    assert_eq!(max_age, Duration::from_secs(24 * 3600), "Default max epoch age should be 24 hours");

    // Should have max_messages_per_epoch (default 10,000)
    let max_messages = config.max_messages_per_epoch();
    assert_eq!(max_messages, 10_000, "Default max messages per epoch should be 10,000");
}
...
async fn test_group_tracks_epoch_start_time() {
    ...
    assert!(epoch_age < Duration::from_secs(1), "Epoch age should be < 1 second for new group");
}
...
async fn test_needs_rekey_after_time_threshold() {
    ...
    let config = GroupConfig::default()
        .with_max_epoch_age(Duration::from_millis(100));
    ...
    assert!(group.needs_rekey(), "Should need rekey after time threshold");
}
...
async fn test_needs_rekey_after_message_threshold() {
    ...
    assert!(group.needs_rekey(), "Should need rekey after message threshold");
}
...
async fn test_needs_rekey_or_logic() {
    ...
    assert!(group.needs_rekey(), "Should need rekey when message threshold hit (even if time threshold not hit)");
}
...
async fn test_rekey_resets_counters() {
    ...
    assert!(group.epoch_age() < Duration::from_secs(1), "Epoch age should reset");
}
...
async fn test_membership_change_triggers_rekey() {
    ...
    assert!(new_epoch > initial_epoch, "Adding member should advance epoch");
}
