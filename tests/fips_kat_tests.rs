...
fn test_fips203_mlkem_encaps_decaps_kat() {
    ...
    let recipient = KeyPair::generate_from_seed(suite, &seed)
        .expect("generate recipient keypair");

    // Encapsulation
    let encaps_seed = hex::decode(
...
fn test_fips_cross_suite_isolation() {
    ...
    let kp_128 = KeyPair::generate_from_seed(suite_128, &seed)
        .expect("generate 128-bit keypair");

    let kp_256 = KeyPair::generate_from_seed(suite_256, &seed)
        .expect("generate 256-bit keypair");

    // Validate different security levels have correct key sizes per FIPS 203
    assert_eq!(kp_128.public_key().to_bytes().len(), 1184,
        "128-bit security level uses ML-KEM-768");
    assert_eq!(kp_256.public_key().to_bytes().len(), 1568,
...
fn test_fips_multiple_key_generation() {
    ...
    let seed1 = hex::decode(
        "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    ).expect("decode seed1");

    let seed2 = hex::decode(
        "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    ).expect("decode seed2");

    let kp1 = KeyPair::generate_from_seed(suite, &seed1)
        .expect("generate keypair 1");

    let kp2 = KeyPair::generate_from_seed(suite, &seed2)
        .expect("generate keypair 2");
...
fn test_all_ciphersuites_fips_compliant() {
    let suites = vec![
        (CipherSuiteId::MLS_128_MLKEM768_AES128GCM_SHA256_MLDSA65, 1184, 1952, 3309),
        (CipherSuiteId::MLS_128_MLKEM768_CHACHA20POLY1305_SHA256_MLDSA65, 1184, 1952, 3309),
        (CipherSuiteId::MLS_256_MLKEM1024_AES256GCM_SHA512_MLDSA87, 1568, 2592, 4627),
    ];
